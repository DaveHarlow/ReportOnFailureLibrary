...
ReportOnFailure helps developers gather debugging information automatically when failures occur. Instead of manually collecting logs, database states, or API responses after a failure, this library allows you to pre-configure data collection that executes only when needed.
...
```sh
dotnet add package ReportOnFailure
```
